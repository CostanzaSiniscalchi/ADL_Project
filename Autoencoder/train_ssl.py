...
from model import MRISequenceSSL
...
def train_ssl(model, dataloader, val_dataloader, optimizer, criterion, epochs=50, patience=5, scheduler=None):
    ...
    for epoch in range(epochs):
        ...
        for batch in tqdm(dataloader, desc=f"Epoch {epoch+1}/{epochs}"):
            ...
            preds = model(scans)  # (B, 3, 32, 256, 240)

            # Match shape: (B, 3, 32, 256, 240)
            loss = criterion(preds, original.squeeze(2))
            ...
            if total_loss == 0:
                make_viz(epoch, './training_viz/', 0, original, scans, preds)
...
if __name__ == "__main__":
    ...
    model = MRISequenceSSL(hidden_dim=128).to(device)

    criterion = nn.MSELoss()
    optimizer = optim.Adam(model.parameters(), lr=1e-4)

    trained_model = train_ssl(
        model, train_loader, val_loader, optimizer, criterion, epochs=50, patience=5)
