...
Filename: model.py
Author: Roshan Kenia
Description: Vision Transformer AutoEncoder architecture used for SSL and prediction on MRI sequences.
"""
...
class ViTAutoEnc(nn.Module):
    ...
    def __init__(
        ...
        if not is_sqrt(patch_size):
            raise ValueError(
                f"patch_size should be square number, got {patch_size}.")
        ...
        self.blocks = nn.ModuleList(
            [
                TransformerBlock(hidden_size, mlp_dim, num_heads,
                                 dropout_rate, qkv_bias, save_attn)
